...
class FakeContext:
    def __init__(self) -> None:
        self.calls: list[tuple[PluginSpec, object]] = []

    def add_fiber(self, spec: PluginSpec, apply: object) -> None:
        self.calls.append((spec, apply))


class ApplyingContext(FakeContext):
    def __init__(self) -> None:
        super().__init__()
        self.applied: list[dict[str, object]] = []

    def add_fiber(self, spec: PluginSpec, apply: object) -> None:
        super().add_fiber(spec, apply)
        apply(self, spec.config)
...
class LoaderTests(unittest.TestCase):
    ...
    def test_loads_two_ordered_bundles_and_calls_module_apply(self) -> None:
        ...
        first.apply = lambda context, config: context.applied.append(dict(config))
        second.apply = lambda context, config: context.applied.append(dict(config))
        ...
        self.assertIs(context.calls[0][1], first.apply)
        self.assertIs(context.calls[1][1], second.apply)
